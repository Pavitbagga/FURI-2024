...
for i, time in enumerate(data.timestamps):
    if (i % every_n_sample) or (time in data.start_times) or (time in data.timeouts):
        ...
        for j in range(6):
            # print(len(data.raw_values[j]), i)
            down_samp_data.raw_values[j].append(data.raw_values[j][i])
            down_samp_data.processed_values[j].append(data.processed_values[j][i])
...
name = data_file.split('.pkl')[0] + '_' + str(target_frequency) + 'Hz' + '_' + str(file_count)
